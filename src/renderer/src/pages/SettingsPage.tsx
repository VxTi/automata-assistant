    ...
    return (
        <ScrollableContainer className="gap-4 p-4" size='sm' elementRef={containerRef}>
            ...
            <MultiSelectionSetting title='Voice recognition language'
                                   ...
                                   currentIndex={Number(Settings.get(Settings.VOICE_RECOGNITION_LANGUAGE))}
                                   props={FadeIn()}
            ...
            <MultiSelectionSetting title="Image generation quality"
                                   description="The quality of the images that are generated by the assistant."
                                   onChange={(value, _) => Settings.set(Settings.IMAGE_GENERATION_QUALITY, value)}
                                   options={Settings.DIFF_IMAGE_SIZES}
                                   props={FadeIn()}
                                   currentIndex={Settings.DIFF_IMAGE_SIZES.indexOf(Settings.get(Settings.IMAGE_GENERATION_QUALITY) as any)}
                                   />
        </ScrollableContainer>
    );
}
...
function MultiSelectionVoiceSetting() {
    ...
    return (
        <MultiSelectionSetting
            ...
            currentIndex={Number(Settings.get(Settings.ASSISTANT_VOICE_TYPE))}
            extraOption={
