...
export interface Tool {
    ...
    function: {
         ...
         * strict is true. Learn more about Structured Outputs in the function calling guide.
         */
        strict?: boolean,
...
export interface CompletionRequest {
    ...
    /**
     * A list of messages comprising the conversation so far. Depending on the model you use, different message types
     * (modalities) are supported, like text, images, and audio.
     */
    messages?: Message [];

    /**
     * ID of the model to use. See the model endpoint compatibility table for details on which models work with the
     * Chat API.
     */
    model?: ModelType;
     ...
     * token. Defaults to an empty object.
     */
    ...
    /**
     * The maximum number of `tokens` that can be generated in the chat completion. This value can be used to control
     ...
     * Note that you will be charged based on the number of generated tokens across all the choices. Keep n as 1 to
     * minimize costs.
     */
    n?: number;
    ...
    /**
     * An object specifying the format that the model must output. Compatible with GPT-4o, GPT-4o mini, GPT-4 Turbo and
     ...
     * token limit, resulting in a long-running and seemingly "stuck" request. Also note that the message content may
     * be partially cut off if `finish_reason="length"`, which indicates the generation exceeded `max_tokens` or the
     ...
     * generates a message. auto means the model can pick between generating a message or calling one or more tools.
...
export interface StreamedChatResponse extends GenericResponse {
    ...
    choices: {
        ...
        delta: {
             ...
             * or a provided stop sequence, length if the maximum number of tokens specified in the request was
             * reached, content_filter if content was omitted due to a flag from our content filters, tool_calls if the
